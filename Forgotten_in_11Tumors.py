...
print('npseed:{}\trdseed:{}'.format(npseed,rdseed))
class Particle:
    # init Particle
    ...
    def updateSingle(self, data, Block, cls, w):
        ...
        for i in range(self.dim):
            ...
            # print('Particle:{},  velocity:{},  pos:{}'.format(self.pID, self.vel[i], self.pos[i]))
...
class Swarm:
    ...
    def __init__(self, data, pNum, Ndim, w0, w1, VRmax, VRmin, impflag, selflag,
        ...
        self.pNum = divSize  # ([10,20,30,40,50])
        self.dim = Ndim
        self.w0 = w0
        self.w1 = w1
        self.Vmax = VRmax
        ...
        self.ParLen = Parlen  # len of each block
        self.cls = classifier
        # self.gbest_pos = np.random.uniform(0, 1, Ndim)
        ...
        self.test_acc = 0
        self.test_bacc = 0
        # init particles, 300(pNum)/12(divNum)=25(divSize)
...
def save_data(data, c):
    ...
    with open('./result/for11_Tumors_{}.txt'.format(run_time), c) as f:
...
if __name__ == '__main__':
    ...
    for t in range(60):
        ...
        data = pd.read_csv('../../dataset/11_Tumors.csv')
        ...
        for f, (train_index, test_index) in enumerate(kf.split(features, labels)):
            ...
            data.to_excel('./hyperresult/HE/FP4.xlsx', index=False)
            save_data(data_dict, 'w')
            # cal average value
